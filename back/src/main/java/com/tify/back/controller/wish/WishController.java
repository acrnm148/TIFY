    ...
    public String addWish(@RequestBody AddWishDto dto) throws IOException, URISyntaxException, NoSuchAlgorithmException, InvalidKeyException {
        ...
        if(result)
        {
            ...
            for (Phonebook tel : tels) {
                ...
                mtemp.setTo(tel.getPhoneNumber());
                String content = user.getUsername() + "님의 "+dto.getCategory()+"을/를 축하해주세요!:tada::partying_face:"
                +dto.getWishTitle()+" 위시에 마음을 전해보세요.:two_hearts:"
                +dto.getEndDate()+"일 까지 아래 링크를 통해 선물을 보낼 수 있습니다.:gift: "
                +"https://i8e208.p.ssafy.io/congrats/"+dto.getWishId()+" "
                +"본 문자는 TIFY티피의 "+user.getUsername()+"님의 주소록에 저장된 "+tel.getName()+"님에게 발송되었습니다."
                +"∞당신을 위한 축하 TIFY∞`";
