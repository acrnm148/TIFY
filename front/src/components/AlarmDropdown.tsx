...
const updateData = (dataid:string,data:Alarm) => {
    data.state = true;
    console.log(data);
    console.log("/test/tify/rkdrlgks321-naver-com/"+dataid);
    return update(
        ref(db, "/test/tify/rkdrlgks321-naver-com/"+dataid),
        data
    );
};
  ...
  const toggleDropdown = () => {
    ...
    alarmsArray.map((val,idx) => {
        if ( alarmsArray[idx].state === false ){
            updateData(alarmsArray[idx].id,alarmsArray[idx]);
        }
    })
    console.log(alarmsArray);
    };

  const mb = ref(db, "/test/tify/rkdrlgks321-naver-com");
