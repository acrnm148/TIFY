        ...
        <div className='thanks-page-container'>
          <button className="back-botton" onClick={() =>(navigate(-1))}> 뒤로가기!!</button>
            <div className='con-thanks-container'>
              <div className='con-card-detail'>
                <div className="arrow" onClick={()=>setGetEmit(!getEmit)}>{left !== null && <NavLink to={`/thanks/${wishId}/${left}`} state={[...state]}><img src={circleArrowL} alt="원형 화살표 좌" /></NavLink>}</div>
                  <ConCardDetail />
                <div className='arrow' onClick={()=>setGetEmit(!getEmit)}>{right !== null && <NavLink  to={`/thanks/${wishId}/${right}`} state={[...state]}><img src={circleArrowR} alt="원형 화살표 우" /></NavLink>}</div>
              </div>
